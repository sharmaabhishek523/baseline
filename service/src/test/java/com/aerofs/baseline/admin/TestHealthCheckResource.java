...
public final class TestHealthCheckResource {
    ...
    public static final class FailureHealthCheck1 implements HealthCheck {
        ...
        @Override
        public HealthCheckStatus check() {
            return HealthCheckStatus.failure("unacceptable");
        }
    }
    ...
    public void shouldReceive_HTTP_OK_WhenHealthCheckSuccessful() throws Exception {
        ...
        Service<ServiceConfiguration> service = new Service<ServiceConfiguration>("test") {
            ...
            @Override
            public void init(ServiceConfiguration configuration, RootEnvironment root, AdminEnvironment admin, ServiceEnvironment service) throws Exception {
                admin.registerHealthCheck("test0", SuccessHealthCheck0.class);
                admin.registerHealthCheck("test1", SuccessHealthCheck1.class);
            }
        };

        // we expect the following ServiceStatus object
        ServiceStatus expected = new ServiceStatus(ImmutableMap.of("test0", HealthCheckStatus.success(), "test1", HealthCheckStatus.success()));
    ...
    public void shouldReceive_HTTP_SERVICE_UNAVAILABLE_WhenHealthCheckFails() throws Exception {
        ...
        Service<ServiceConfiguration> service = new Service<ServiceConfiguration>("test") {
            ...
            public void init(ServiceConfiguration configuration, RootEnvironment root, AdminEnvironment admin, ServiceEnvironment service) throws Exception {
                admin.registerHealthCheck("test0", SuccessHealthCheck0.class);
                admin.registerHealthCheck("test1", FailureHealthCheck0.class);
            }
        };

        // we expect the following ServiceStatus object
        ServiceStatus expected = new ServiceStatus(ImmutableMap.of("test0", HealthCheckStatus.success(), "test1", HealthCheckStatus.failure()));
    ...
    public void shouldReceive_HTTP_SERVICE_UNAVAILABLE_WithDetailedFailureInformationWhenHealthCheckFails() throws Exception {
        ...
        Service<ServiceConfiguration> service = new Service<ServiceConfiguration>("test") {
            ...
            public void init(ServiceConfiguration configuration, RootEnvironment root, AdminEnvironment admin, ServiceEnvironment service) throws Exception {
                admin.registerHealthCheck("test0", SuccessHealthCheck0.class);
                admin.registerHealthCheck("test1", FailureHealthCheck1.class);
            }
        };

        // we expect the following ServiceStatus object
        ServiceStatus expected = new ServiceStatus(ImmutableMap.of("test0", HealthCheckStatus.success(), "test1", HealthCheckStatus.failure("unacceptable")));
    ...
    public void shouldReceive_HTTP_SERVICE_UNAVAILABLE_WithExceptionMessageWhenHealthCheckFails() throws Exception {
        ...
        Service<ServiceConfiguration> service = new Service<ServiceConfiguration>("test") {
            ...
            public void init(ServiceConfiguration configuration, RootEnvironment root, AdminEnvironment admin, ServiceEnvironment service) throws Exception {
                admin.registerHealthCheck("test0", SuccessHealthCheck0.class);
                admin.registerHealthCheck("test1", FailureHealthCheck2.class);
            }
        };

        // we expect the following ServiceStatus object
        ServiceStatus expected = new ServiceStatus(ImmutableMap.of("test0", HealthCheckStatus.success(), "test1", HealthCheckStatus.failure("failed with exception")));
